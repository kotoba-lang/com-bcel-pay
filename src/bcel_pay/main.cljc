(ns bcel-pay.main
  "Kotodama WASM entrypoint for the Bcel_pay clean-room actor (L4) — Clojure port.
  ...
  facts (`bcel_pay.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bcel_pay")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bcel_pay.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bcel_pay-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bcel_pay-compat/ready)
